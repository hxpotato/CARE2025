...
class ResidualMLPHead(nn.Module):
    def __init__(self, in_dim):
        ...
        # 残差映射：输入到128维，如果in_dim != 128，则用线性映射
...
#
#
# def eva(ckpt_path,val_dir=None, batch_size=1, device_id=0,task = 'cirrhosis'):  # 加上 early_stop 参数
#     # 在 main 函数中加入：
#     device = torch.device(f"cuda:{device_id}" if torch.cuda.is_available() else "cpu")
#
...
#     model.load_state_dict(state_dict)  # 不要赋值给 model
#
...
if __name__ == "__main__":
    ...
    # main(r"/data/sdd/LiQA/Normalization",use_contrast=False, device_id=0,task = 'cirrhosis')  # 关闭早停
    # main(r"/data/sdd/LiQA/Normalization",use_contrast=False, device_id=0,task = 'fibrosis')  # 关闭早停
